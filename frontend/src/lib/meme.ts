...
export function generateMemeText(input: MemeInput): string {
  const { sigungu, area, totalMonths, reachable, age } = input;
  const loc = sigungu || '이 동네';
  const pyeong = area ? `${Math.round(area / 3.3058)}평` : '';
  ...
  if (totalMonths === 0) {
    return pick([
      ...
      `${loc}${pyeong ? ` ${pyeong}` : ''}, 오늘이 가장 싼 날입니다.`,
    ]);
  }

  const yrs = Math.ceil(totalMonths / 12);

  if (totalMonths <= 12) {
    return pick([
      '1년도 안 걸립니다! 커피 좀 줄이면 더 빨라요.',
      '올해 안에 가능합니다. 연말 집들이 계획 세우세요.',
      `곧입니다! ${loc}${pyeong ? ` ${pyeong}` : ''}이 기다리고 있어요.`,
    ]);
  }
